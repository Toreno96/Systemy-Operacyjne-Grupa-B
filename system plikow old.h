...
private:
	...
	std::array <int, 10> sectorID; // wartosc ujemna – nieprzypisany sektor do pliku​

public:
	FCB(std::array <char, 8> filename_, std::array <char, 3> type_, std::array <int, 10> sectorID_) : filename(filename_), type(type_), sectorID(sectorID_) {}
	bool add_sector(int sectorID_) {}
	std::array <int, 10> read_sectors() {}
};
